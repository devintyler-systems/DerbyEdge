...
CATALOG: list[tuple] = [
    ...
    ("career_win_pct",      "IMPLEMENTED", "entry",
     ...
     "null_if_starts_zero", 1, 1, "Diagnostic/audit only; excluded from baseline scoring"),
    ("recent_finish_percentile_w", "IMPLEMENTED", "horse_pre_race",
     "valid starts only: finish_percentile = 1 - ((finish_position - 1) / (field_size - 1)); recency-weighted across valid historical starts",
     "horse_starts: non-scratch; start_date < target race date; field_size present >= 2; finish_position present and 1 <= finish_position <= field_size",
     "neutral_0.50_if_insufficient", 1, 1,
     "Invalid/singleton records contribute no observed percentile; absent valid evidence lowers coverage and sparse valid evidence is blended toward the neutral prior; target-date/target-race records are excluded."),
    ("starts_last_90d", "IMPLEMENTED", "entry",
     "count valid non-scratch starts with 0 < target_date-start_date <= 90",
     "horse_starts.start_date, is_scratch", "zero_if_none", 1, 1, ""),
    ("class_delta_last_to_today", "IMPLEMENTED", "entry",
     "(same-circuit recognized last_class_rank - today_class_rank) / 100",
     "horse_starts.race_class_raw, track_code, race_cards.race_class",
     "neutral_0_low_confidence_if_unconditioned", 1, 1, ""),
    ...
    ("workout_readiness_score_v2", "IMPLEMENTED", "entry",
     "coverage blend of recency .35, cadence .30, distance progression .20, source rank .10, compatible-cohort time .05",
     "workouts pre-race rows", "neutral_0.50_if_sparse", 1, 1, ""),
    # ── Connections ──────────────────────────────────────────────────────────
    ...
    ("distance_fit_eb", "IMPLEMENTED", "entry",
     "(matching-distance wins + 2*0.50)/(matching-distance starts+2)",
     "horse_starts", "neutral_0.50_if_none", 1, 1, ""),
    ("surface_fit_eb", "IMPLEMENTED", "entry",
     "(matching-surface wins + 2*0.50)/(matching-surface starts+2)",
     "horse_starts", "neutral_0.50_if_none", 1, 1, ""),
    ("surface_distance_finish_percentile_w", "IMPLEMENTED", "entry",
     "recency-weighted field-adjusted finish percentile with two neutral pseudo-observations",
     "horse_starts", "neutral_0.50_if_none", 1, 1, ""),
