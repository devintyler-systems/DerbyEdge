...
def devig_proportional(decimals: list[float | None]) -> list[float | None]:
    ...
    Best fit for win-pool style markets. Preserves rank ordering.
    Inputs of None are passed through as None and excluded from the sum.
    """
...
def devig_power(decimals: list[float | None], tol: float = 1e-6,
    ...
    More accurate than proportional when overround is concentrated on favorites.
    Uses bisection on k in [0.5, 2.0].
    """
    raws = [(i, decimal_to_implied_prob(d)) for i, d in enumerate(decimals)]
    valid = [(i, p) for i, p in raws if p is not None and p > 0]
    if not valid:
...
def edge(model_prob: float, market_prob: float) -> float:
    """Multiplicative edge: (model - market) / market. >0 = positive expected value."""
    if market_prob <= 0:
        return 0.0
...
def kelly_fraction(model_prob: float, decimal_odds: float, cap: float = 0.05) -> float:
    """Fractional Kelly bankroll allocation, capped (default 5% — quarter Kelly-ish for racing).

    f* = (bp - q) / b where b = decimal-1, p = model_prob, q = 1-p.
    """
    if decimal_odds <= 1.0:
        return 0.0
    b = decimal_odds - 1.0
...
def bet_tag(model_prob: float, market_prob: float, decimal_odds: float,
    ...
    """Classify the wager:
        STRONG_PLAY  -> edge >= strong_edge AND EV > 0 AND model_prob >= 0.06
        VALUE_PLAY   -> edge >= min_edge AND EV > 0
        WATCH        -> 0 < edge < min_edge
        FADE         -> model_prob < market_prob * 0.7 AND market_prob >= 0.10
        PASS         -> otherwise
    """
